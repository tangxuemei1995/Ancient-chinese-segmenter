...
#             exit()
            ...
            # print('total_loss',total_loss.size())#1552 = 16*97
            total_loss = total_loss.reshape(sequence_output.size(0),sequence_output.size(1))
            # print('total_loss',total_loss.size())# reshape成batch * sequence_length 16*97
            valid = attention_mask.sum(1).float()
            # print(total_loss)
#             print(valid)
            sort_loss, idx = total_loss.max(1)
        ...
        if self.classifier_cls is not None:
            ...
            return sort_loss, total_loss, tag_seq, index_
